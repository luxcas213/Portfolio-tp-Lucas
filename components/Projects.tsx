  ...
  return (
    ...
    >
      <div className="container mx-auto max-w-6xl py-20">
        <motion.h2
          className="text-4xl md:text-6xl font-extrabold text-[#3a4151] mb-4"
          initial={{ opacity: 0, y: -20 }}
          animate={{ opacity: 1, y: 0 }}
        ...
        <motion.p
          className="text-lg md:text-xl text-[#3a4151] mb-12"
          initial={{ opacity: 0 }}
          animate={{ opacity: 1 }}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {projects.map((project, index) => (
            <motion.div
              key={index}
              className="bg-white/90 backdrop-blur-md rounded-lg p-6 border border-gray-200 hover:border-[#f3023d] transition hover:shadow-2xl hover:scale-105"
              initial={{ opacity: 0, y: 30 }}
              animate={{ opacity: 1, y: 0 }}
              transition={{ duration: 0.5, delay: 0.4 + index * 0.1 }}
              whileHover={{ scale: 1.05 }}
            >
              <h3 className="text-xl font-semibold text-[#3a4151] mb-3">
                {project.title}
              </h3>
              <p className="text-[#3a4151] mb-4 text-sm">
                {project.description}
              </p>
              <div className="flex flex-wrap gap-2 mb-4">
                {project.technologies.map((tech, techIndex) => (
                  <span
                    key={techIndex}
                    className="text-xs px-3 py-1 bg-[#f3023d] text-white rounded-full font-medium"
                  >
                    {tech}
                  </span>
                ))}
              ...
              <a
                ...
                className="inline-flex items-center text-sm font-medium text-[#f3023d] hover:text-[#0e0e0e] transition"
              >
                Ver proyecto
                <svg
